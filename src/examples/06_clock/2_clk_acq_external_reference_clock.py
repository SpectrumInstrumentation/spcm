...
2_clk_acq_external_reference_clock.py

An example to show a simple recording while using the external referenceclock mode.
...
with spcm.Card(card_type=spcm.SPCM_TYPE_AI) as card:            # if you want to open the first card of a specific type
    ...
    clock.mode(spcm.SPC_CM_EXTREFCLOCK)    # clock mode external reference clock
    clock.reference_clock(10 * units.MHz)  # tell the driver the frequency of the used external clock
