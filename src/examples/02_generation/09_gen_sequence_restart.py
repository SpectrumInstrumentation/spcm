...
9_gen_sequence_restart.py

Shows a simple sequence restart mode example using only the few necessary commands.
- output on channel 0
- 10% of the maximum sampling rate of the card

Example for analog replay cards (AWG) for the the M4i and M4x card-families.
...
def setup_sequence(sequence : spcm.Sequence):
    ...
    #  part 2: sync, sin 3
    #  part 3: sync, sin 4
    #  part 4: dc-level and restart at the next trigger

    # sine
    part0_sync = sequence.add_step(segment_sync, loops=1)
    part0_sin1 = sequence.add_step(segment_sin1, loops=20)

    # cosine
    part1_sync = sequence.add_step(segment_sync, loops=1)
    part1_sin2 = sequence.add_step(segment_sin2, loops=20)

    # inverted sine
    part2_sync = sequence.add_step(segment_sync, loops=1)
    part2_sin3 = sequence.add_step(segment_sin3, loops=20)

    # inverted cosine
    part3_sync = sequence.add_step(segment_sync, loops=1)
    part3_sin4 = sequence.add_step(segment_sin4, loops=20)
    ...
    part0_sin1.set_transition(part1_sync)
    
    # The second part of the sequence
    part1_sync.set_transition(part1_sin2)
    part1_sin2.set_transition(part2_sync)
    
    # The third part of the sequence
    part2_sync.set_transition(part2_sin3)
    part2_sin3.set_transition(part3_sync)
    
    # The fourth part of the sequence
    part3_sync.set_transition(part3_sin4)
    part3_sin4.set_transition(final_dc)

    # This is the final step of the sequence, now waiting for a trigger to restart the whole sequence
    final_dc.set_transition(part0_sync)
...
with spcm.Card(card_type=spcm.SPCM_TYPE_AO, verbose=False) as card:          # if you want to open the first card of a specific type
    ...
    # setup card mode
    card.card_mode(spcm.SPC_REP_STD_SEQUENCERESTART)
    ...
    # set up trigger to start each sequence
